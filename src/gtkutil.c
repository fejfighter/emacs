...
#else
#define xp gtk4
typedef struct gtk4_output xp_output;
#endif
...
#ifdef HAVE_GTK4
      GdkPixbuf *pb;
      pb = gdk_pixbuf_get_from_surface(surface, 0, 0,
				       cairo_image_surface_get_width(surface),
				       cairo_image_surface_get_height(surface));
      g_object_unref(pb);
      if (! old_widget)
	{
	  old_widget = GTK_IMAGE (gtk_image_new_from_pixbuf(pb));
	}
      else
	{
...
xg_set_cursor (GtkWidget *w, GdkCursor *cursor)
{
  ...
  for ( ; children; children = g_list_next (children))
    gdk_window_set_cursor (GTK_WINDOW (children->data), cursor);
#endif
}
...
#else
    success_p = gtk4_parse_color (buf, color) != 0;
#endif
#else
...
xg_hide_tooltip (struct frame *f)
{
  ...
  if (f->output_data.xp->ttip_window)
      ...
      if (g_object_get_data (G_OBJECT (win), "restore-tt"))
	{
	  ...
	  GdkDisplay *dpy = gdk_window_get_display (gwin);
#endif
	  GtkSettings *settings = gtk_settings_get_for_display(dpy);
...
{
  {
    ...
    GTK4_TRACE("set go left %d top %d scal %d lef/s %d top/s %d",
	     f->left_pos, f->top_pos, scale, f->left_pos/scale, f->top_pos/scale);
  }
  if (f->size_hint_flags & (USPosition | PPosition))
	...
	{
#endif
	  GTK4_TRACE("never get ehree set go left %d top %d scal %d lef/s %d top/s %d",
		     f->left_pos, f->top_pos, scale, f->left_pos/scale, f->top_pos/scale);
...
# if ! GTK_CHECK_VERSION (3, 22, 0)
	...
	{
	  GTK4_TRACE("set go left %d top %d scal %d lef/s %d top/s %d",
		 f->left_pos, f->top_pos, 4, f->left_pos/scale, f->top_pos/scale);
...
xg_frame_resized (struct frame *f, int pixelwidth, int pixelheight)
{
  ...
  GTK4_TRACE("xg_frame_resized: pixel:--------------------");
  GTK4_TRACE("xg_frame_resized: pixel: %dx%d, text: %dx%d", pixelwidth, pixelheight, width, height);
  GTK4_TRACE("xg_frame_resized: pixel:--------------------");
  ...
  GTK4_TRACE("width: %d -> %d.", FRAME_TEXT_WIDTH(f), width);
  GTK4_TRACE("height: %d -> %d.", FRAME_TEXT_HEIGHT(f), height);
  GTK4_TRACE("pixelwidth: %d -> %d.", FRAME_PIXEL_WIDTH(f), pixelwidth);
  GTK4_TRACE("pixelheight: %d -> %d.", FRAME_PIXEL_HEIGHT(f), pixelheight);
...
xg_create_frame_widgets (struct frame *f)
{
...
#ifndef HAVE_GTK4  // gtk_plug not found.
  ...
  {
#ifdef HAVE_GTK4
    GdkDisplay *dpy = gtk_widget_get_display (wtop);
    GtkSettings *gs = gtk_settings_get_for_display (dpy);
  ...
  if (FRAME_GTK_OUTER_WIDGET (f1) && FRAME_GTK_OUTER_WIDGET (f2))
    {
      ...
      x_sync (f1);
#else
      gdk_surface_restack (gwin1, gwin2, above_flag);
      //gdk_flush();
#endif
    }
  unblock_input ();
...
xg_set_skip_taskbar (struct frame *f, Lisp_Object skip_taskbar)
{
  ...
  if (FRAME_GTK_WIDGET (f))
    ...
    /*   (gtk_widget_get_surface (FRAME_GTK_OUTER_WIDGET (f)), */
    /*    NILP (skip_taskbar) ? FALSE : TRUE); */
  ...
  if (FRAME_GTK_WIDGET (f))
    {
      ...
      gtk_window_set_type_hint (gwin, GDK_SURFACE_TYPE_HINT_TOOLTIP);
...
#else
      gtk_box_insert_child_after (cur_box, wvbox, NULL);
      gtk_box_insert_child_after (GTK_BOX (wvbox), whbox_up, NULL);
      gtk_box_insert_child_after (GTK_BOX (wvbox), whbox_down, NULL);
#endif
...
#else
	  gtk_box_insert_child_after (wvbox, gtk_label_new (""), NULL);
	  gtk_box_insert_child_after (wvbox, w, NULL);
#endif
...
#else
	  gtk_box_insert_child_after (cur_box, w, NULL);
#endif
...
#else
  gtk_box_insert_child_after (GTK_BOX (wbox), wtoggle, NULL);
#endif
  if (x_gtk_file_dialog_help_text)
    {
#ifndef HAVE_GTK4
...
#else
      gtk_box_insert_child_after (GTK_BOX (wbox), wmessage, NULL);
#endif
    }
  ...
  FOR_EACH_FRAME (rest, frame)
    {
      ...
      if ((FRAME_X_P (f) || FRAME_GTK4_P (f)) && FRAME_GTK_OUTER_WIDGET (f))
	{
	  struct xg_frame_tb_info *tbinfo
	    = g_object_get_data (G_OBJECT (FRAME_GTK_OUTER_WIDGET (f)),
				 TB_INFO_KEY);
...
#else
  gtk_box_insert_child_after (GTK_BOX (wbox), wlbl, NULL);
  gtk_box_insert_child_after (GTK_BOX (wbox), wkey, NULL);
#endif
...
#else
  gtk_box_insert_child_after (GTK_BOX (x->vbox_widget), x->menubar_widget, NULL);
  gtk_box_reorder_child_after (GTK_BOX (x->vbox_widget), x->menubar_widget, 0);
#endif
...
#ifndef HAVE_GTK3
  gtk_scrollbar_set_update_policy (wscroll, GTK_UPDATE_CONTINUOUS);
#endif
  ...
  if (wscroll)
    {
      ...
      GTK4_TRACE("top %d, left %d, width %d height %d scale %d", top, left, width, height, scale);
      ...
      else
	{
	  ...
	  //#endif
	  GTK4_TRACE("width %d height %d", width, height);
...
#else
	  gtk4_clear_area (f, oldx, oldy, oldw, oldh);
#endif
	}
#ifndef HAVE_GTK4
      if (!hidden)
...
static int
int_gtk_range_get_value (GtkScrollbar *range)
{
				 ...
				 int whole)
{
  ...
  GTK4_TRACE("xg_set_toolkit_scroll_bar_thumb: ----------------------------------");
  GTK4_TRACE("xg_set_toolkit_scroll_bar_thumb: %p, %d, %d, %d.", bar, portion, position, whole);
  if (wscroll && bar->dragging == -1)
    {
      ...
      adj = gtk_scrollbar_get_adjustment (wscroll);
      ...
      else
	{
      ...
      GTK4_TRACE("xg_set_toolkit_scroll_bar_thumb: position=%d, portion=%d, whole=%d", position, portion, whole);
      GTK4_TRACE("xg_set_toolkit_scroll_bar_thumb: top=%f, shown=%f", top, shown);

      GTK4_TRACE("xg_set_toolkit_scroll_bar_thumb: shown*range=%f", shown * XG_SB_RANGE);
      size = clip_to_bounds (1, shown * XG_SB_RANGE, XG_SB_RANGE);
      GTK4_TRACE("xg_set_toolkit_scroll_bar_thumb: size=%d.", size);
      GTK4_TRACE("xg_set_toolkit_scroll_bar_thumb: top*range=%f.", top * XG_SB_RANGE);
      GTK4_TRACE("xg_set_toolkit_scroll_bar_thumb: max-size=%d.", XG_SB_MAX - size);
      value = clip_to_bounds (XG_SB_MIN, top * XG_SB_RANGE, XG_SB_MAX - size);
      GTK4_TRACE("xg_set_toolkit_scroll_bar_thumb: value=%d.", value);

      /* Assume all lines are of equal size.  */
      new_step = size / max (1, FRAME_LINES (f));

      old_size = gtk_adjustment_get_page_size (adj);
      GTK4_TRACE("xg_set_toolkit_scroll_bar_thumb: old_size=%d, size=%d", old_size, size);
      if (old_size != size)
	{
	  int old_step = gtk_adjustment_get_step_increment (adj);
	  GTK4_TRACE("xg_set_toolkit_scroll_bar_thumb: old_step=%d, new_step=%d", old_step, new_step);
	...
	if (int_gtk_range_get_value (wscroll) != value)
	  gtk_range_set_value (wscroll, (gdouble)value);
...
#else
      && gdk_event_get_event_type(event) == GDK_BUTTON_PRESS /* && gdk_event_get_button(event) < 4 */
#endif
      )
    {
...
#else
	surface = gdk_device_get_surface_at_position (gdev, NULL, NULL);
#endif
#else
...
#else
	   && ((gdk_event_get_event_type(event) == GDK_BUTTON_RELEASE /* && event->button.button < 4 */)
	       || gdk_event_get_event_type(event) == GDK_MOTION_NOTIFY)
#endif
	   )
    {
...
#else
  event.modifiers = gtk4_gtk_to_emacs_modifiers (FRAME_DISPLAY_INFO (f), mod);
#endif
  kbd_buffer_store_event (&event);
...
xg_pack_tool_bar (struct frame *f, Lisp_Object pos)
{
  ...
  if (into_hbox)
	...
	gtk_box_reorder_child (GTK_BOX (x->hbox_widget),
			       ...
			       0);
#else
      gtk_box_insert_child_after (GTK_BOX (x->hbox_widget), top_widget, NULL);

      if (EQ (pos, Qleft))
	gtk_box_reorder_child_after (GTK_BOX (x->hbox_widget),
			       top_widget,
			       0);
#endif
	...
	gtk_box_reorder_child (GTK_BOX (x->vbox_widget),
			       ...
			       vbox_pos);
#else
      gtk_box_insert_child_after (GTK_BOX (x->vbox_widget), top_widget, NULL);

      if (EQ (pos, Qtop))
	gtk_box_reorder_child_after (GTK_BOX (x->vbox_widget),
			       top_widget,
			       NULL);
...
#else
  if (wimage && !text_image)
    gtk_box_insert_child_after (GTK_BOX (vb), wimage, NULL);
  if (label)
    gtk_box_insert_child_after (GTK_BOX (vb), gtk_label_new (label), NULL);
  if (wimage && text_image)
    gtk_box_insert_child_after (GTK_BOX (vb), wimage, NULL);
#endif
...
#else
  screen = gtk_widget_get_display(GTK_WIDGET (wtoolbar));
  icon_theme = gtk_icon_theme_get_for_display (screen);
#endif
  /* Are we up to date? */
	  ...
	  if (EQ (style, Qtext))
	  ...
	  else if (icon_name)
	    {
	      ...
	      g_object_set_data_full (G_OBJECT (w), XG_TOOL_BAR_ICON_NAME,
				      ...
				      (GDestroyNotify) xfree);
#else
	      w = gtk_image_new_from_icon_name (icon_name);
	      g_object_set_data_full (G_OBJECT (w), XG_TOOL_BAR_ICON_NAME,
				      (gpointer) xstrdup (icon_name),
				      (GDestroyNotify) xfree);
