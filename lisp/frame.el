...
(declare-function gtk4-frame-geometry "gtk4fns.c" (&optional frame))
...
(declare-function gtk4-frame-edges "gtk4fns.c" (&optional frame type))
...
(declare-function gtk4-mouse-absolute-pixel-position "gtk4fns.c")
...
position (0, 0) of the selected frame's terminal."
  (let ((frame-type (framep-on-display)))
    (cond
     ...
     ((eq frame-type 'gtk4)
      (gtk4-mouse-absolute-pixel-position))
     (t
      (cons 0 0)))))

(declare-function gtk4-set-mouse-absolute-pixel-position "gtk4fns.c" (x y))
...
position (0, 0) of the selected frame's terminal."
  (let ((frame-type (framep-on-display)))
    (cond
     ((eq frame-type 'gtk4)
      (gtk4-set-mouse-absolute-pixel-position x y))
     ((eq frame-type 'ns)
...
(declare-function gtk4-frame-list-z-order "gtk4fns.c" (&optional display))
...
Return nil if DISPLAY contains no Emacs frame."
  (let ((frame-type (framep-on-display display)))
    (cond
     ...
     ((eq frame-type 'gtk4)
      (gtk4-frame-list-z-order display)))))
...
(declare-function gtk4-frame-restack "gtk4fns.c" (frame1 frame2 &optional above))
  ...
  (if (and (frame-live-p frame1)
      ...
      (let ((frame-type (framep-on-display frame1)))
        (cond
         ...
         ((eq frame-type 'gtk4)
          (gtk4-frame-restack frame1 frame2 above))))
...
DISPLAY can be a display name, a frame, or nil (meaning the selected
frame's display)."
  (not (null (memq (framep-on-display display) '(x w32 ns gtk4)))))
...
the keybinding RET and [return]."
  (let ((frame-type (framep-on-display display)))
    (or (memq frame-type '(x w32 ns pc gtk4))
...
monitor, use `display-monitor-attributes-list'."
  (and (memq (framep-on-display display) '(x w32 ns gtk4))
...
(declare-function ns-display-monitor-attributes-list "nsfns.m"
		  (&optional terminal))
(declare-function gtk4-display-monitor-attributes-list "gtk4fns.c"
		  (&optional terminal))
...
monitors."
  (let ((frame-type (framep-on-display display)))
    (cond
     ...
     ((eq frame-type 'gtk4)
      (gtk4-display-monitor-attributes-list display))
     (t
